...
var T = {
  en: {
    ...
    mSummary: "Summary", mAction: "Requested action",
    mLinkDrive: "Original file in Drive", mLinkGmail: "Original email in Gmail",
    mLinkForm: "Original form submission", mLinkOther: "Original document",
  ...
  he: {
    ...
    mSummary: "תקציר", mAction: "הפעולה הנדרשת",
    mLinkDrive: "הקובץ המקורי בדרייב", mLinkGmail: "המייל המקורי בג׳ימייל",
    mLinkForm: "שליחת הטופס המקורית", mLinkOther: "המסמך המקורי",
...
/* WF-4 names the link after the channel the document arrived on, so a Gmail
 * item is not announced as a file in Drive. These four have to stay in step
 * with the expression in the two Gmail nodes of WF-4-router.json. */
function linkLabel(channel){
  return t({ drive:"mLinkDrive", gmail:"mLinkGmail", form:"mLinkForm" }[channel] || "mLinkOther");
}
